...
public class OrderItemServiceImpl implements OrderItemService {
    ...
    public Response filterOrderItems(OrderStatus status, LocalDateTime startDate, LocalDateTime endDate, Long itemId, Pageable pageable) {
        // Crear una especificación para filtrar los elementos de pedido según el estado, la fecha de creación y el ID del artículo
        Specification<OrderItem> spec = Specification.where(OrderItemSpecification.hasStatus(status))
                .and(OrderItemSpecification.createdBetween(startDate, endDate))
                .and(OrderItemSpecification.hasItemId(itemId));

        // Buscar todos los elementos de pedido que cumplan con la especificación y paginarlos
        Page<OrderItem> orderItemPage = orderItemRepo.findAll(spec, pageable);

        // Si no se encuentran elementos de pedido, lanzar una excepción NotFoundException
        if (orderItemPage.isEmpty()) {
            throw new NotFoundException("No Order Found");
        }

        // Mapear los elementos de pedido a DTOs
        List<OrderItemDto> orderItemDtos = orderItemPage.getContent().stream()
                .map(entityDtoMapper::mapOrderItemToDtoPlusProductAndUser)
                .collect(Collectors.toList());

        // Construir y devolver la respuesta con el estado 200, la lista de DTOs, el total de páginas y el total de elementos
        return Response.builder()
                .status(200)
